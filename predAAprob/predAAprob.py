...
firstCondSecondFiles = glob.glob("/home/kuangyu/workspace/aaCodonProbPred/coefs/aa_firstCondSecond_coefs/*.txt")
secondCondFirstFiles = glob.glob("/home/kuangyu/workspace/aaCodonProbPred/coefs/aa_secondCondFirst_coefs/*.txt")
...
class AApairwise:
    ...
    def getAAprob_onePos(self, pos):
		...
		for firstAA_pos in beforeAAs:
			...
			currLogP += math.log(self.secondCondFirstSolvers[self.aaDict[firstAA_pos]].getCatProb(aa, tmpX))

		nBefore = len(beforeAAs)
	    else:
		nBefore = 0
		...
		for secondAA_pos in afterAAs:
			...
			currLogP += math.log(self.firstCondSecondSolvers[self.aaDict[secondAA_pos]].getCatProb(aa, tmpX))

	    	nAfter = len(afterAAs)
	    else:
		nAfter = 0

	    currLogP = currLogP - (nBefore + nAfter - 1) * math.log(firstOrderProb)
