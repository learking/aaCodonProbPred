...
class AllPW:
    ...
    def aa_TD2str(self):
        ...
        TD_invsq_list = []
        for aa in aaList:
            currETD = self.sumList_invsq(self.aa_TD_dict[aa])
            TD_invsq_list.append(currETD)
        TD_invsq_str = ",".join(map(str, TD_invsq_list))
        finalStr = self.codon + "," + self.AA + "," + self.sa + "," + TD_invsq_str
        return finalStr

    def aa_TD2str_cutoff(self, cutoffTD):
        aaList = ['A','C','D','E','F','G','I','H','K','L','M','N','P','Q','R','S','T','V','W','Y']
        TD_invsq_list = []
        for aa in aaList:
            currETD = self.sumList_invsq_cutoff(self.aa_TD_dict[aa], cutoffTD)
            TD_invsq_list.append(currETD)
        TD_invsq_str = ",".join(map(str, TD_invsq_list))
        finalStr = self.codon + "," + self.AA + "," + self.sa + "," + TD_invsq_str
        return finalStr
    ...
    def sumList_invsq_cutoff(self, inputList, cutoffTD):
        sumValue = 0
        for val in inputList:
            if val < cutoffTD:
                sumValue += math.pow(1.0/val, 2)
