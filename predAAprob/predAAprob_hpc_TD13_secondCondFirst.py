...
firstCondSecondFiles = glob.glob("/home/kwang2/result/newData_Feb3_2014/aa_firstCondSecond_TD13_coefs/*.txt")
secondCondFirstFiles = glob.glob("/home/kwang2/result/newData_Feb3_2014/aa_secondCondFirst_TD13_coefs/*.txt")

invsqCutoff=math.pow(1.0/13.0, 2)
    ...
    def getAAprob_onePos(self, pos):
			...
			tmpX = self.pairDict[(firstAA_pos, pos)]
                        ...
                        if tmpX[4] > invsqCutoff:
                            if useFirstCondSecond:
                                ...
                                currLogP += math.log(self.secondCondFirstSolvers[self.aaDict[firstAA_pos]].getCatProb(aa, tmpX)) - math.log(firstOrderProb)
		...
		for secondAA_pos in afterAAs:
			tmpX = self.pairDict[(pos, secondAA_pos)]
                        if tmpX[4] > invsqCutoff:
                            if useFirstCondSecond:
                                currLogP += math.log(self.firstCondSecondSolvers[self.aaDict[secondAA_pos]].getCatProb(aa, tmpX)) - math.log(firstOrderProb)
                            else:
